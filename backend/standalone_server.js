...
var require_tickets = __commonJS({
  "routes/tickets.js"(exports2, module2) {
    ...
    router.post("/", async (req, res) => {
      try {
        const { jobId, subject, message, priority } = req.body;
        if (!jobId || !subject || !message) return res.status(400).json({ error: "jobId, subject, and message are required" });
        const ticket = await SupportTicket.create({ jobId, userId: req.user._id, userName: req.user.name, userRole: req.user.role, subject: subject.trim(), message: message.trim(), priority: priority || "Medium" });
    ...
    router.get("/job/:jobId", async (req, res) => {
      try {
        ...
        if (req.user.role !== "superadmin") filter.userId = req.user._id;
    ...
    router.get("/", requireRole("superadmin"), async (req, res) => {
      try {
        const tickets = await SupportTicket.find().populate("jobId", "title").sort({ createdAt: -1 });
        res.json(tickets);
      } catch (err) { res.status(500).json({ error: err.message }); }
    });
    router.put("/:id", requireRole("superadmin"), async (req, res) => {
    ...
    router.delete("/:id", async (req, res) => {
      try {
        ...
        if (!ticket) return res.status(404).json({ error: "Ticket not found" });
        if (req.user.role !== "superadmin" && String(ticket.userId) !== String(req.user._id)) return res.status(403).json({ error: "Not allowed" });
