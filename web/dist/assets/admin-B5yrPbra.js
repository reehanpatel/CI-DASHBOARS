import"./api-Ctf9aBrp.js";let Y=null,x={personnel:[],clients:[],services:[],salaryGrades:[],salaryAssignments:[]},M=null,T={tab:"dashboard",period:"month",jobsFilter:"all",ticketsFilter:"all"},g={title:"",clientId:"",serviceIds:[],date:new Date().toISOString().slice(0,10),completion:"",value:"",desc:"",assignments:[{personId:"",percent:100,hours:""},{personId:"",percent:0,hours:""}]};async function ge(){initTheme(),Y=requireAuth("superadmin"),Y&&(await _(),K())}async function _(){const[t,s,e]=await Promise.all([apiGet("/personnel"),apiGet("/clients"),apiGet("/services")]);x.personnel=t,x.clients=s,x.services=e}function V(){const t=x.personnel.find(a=>/mansi/i.test(a.name)),s=x.personnel.find(a=>/urna/i.test(a.name)),e=[];return t&&e.push({personId:t._id,percent:"",hours:""}),s&&e.push({personId:s._id,percent:"",hours:""}),e.length===0&&e.push({personId:"",percent:"",hours:""}),e}function pe(t){const s=x.personnel.find(e=>e._id===t);return s?s.name:"—"}function W(t){const s=x.clients.find(e=>e._id===t);return s?s.name:"—"}const Q=[{key:"dashboard",label:"Dashboard",icon:"📊"},{key:"dailytasks",label:"Daily Tasks",icon:"✅"},{key:"logjob",label:"Log a Job",icon:"➕"},{key:"jobs",label:"All Jobs",icon:"📁"},{key:"tickets",label:"Support Tickets",icon:"🎫"},{key:"byclient",label:"By Client",icon:"💼"},{key:"byperson",label:"By Person",icon:"👥"},{key:"accounts",label:"Accounts",icon:"🏢"},{key:"targets",label:"Targets",icon:"🎯"},{key:"salaries",label:"Salaries",icon:"💰"},{key:"users",label:"Users",icon:"👤"},{key:"manage",label:"Manage",icon:"⚙️"}];function K(){const t=document.getElementById("app"),s=Q.find(e=>e.key===T.tab)||Q[0];t.innerHTML=renderAppShell({user:Y,currentRole:"superadmin",activeTab:T.tab,tabs:Q,title:s.label,subtitle:"Productivity & Revenue Intelligence"}),bindAppShellEvents(e=>{T.tab=e,K()}),h()}window.ci360NavTab=t=>{T.tab=t,K()};async function h(){const t=document.getElementById("content");if(t){t.innerHTML=renderSkeletonCards(4);try{T.tab==="dashboard"?await be(t):T.tab==="dailytasks"?await ve(t):T.tab==="logjob"?O(t):T.tab==="jobs"?await ke(t):T.tab==="tickets"?await J(t):T.tab==="byclient"?await $e(t):T.tab==="byperson"?await me(t):T.tab==="accounts"?await Se(t):T.tab==="targets"?await Ie(t):T.tab==="salaries"?await Ae(t):T.tab==="settings"||T.tab==="manage"?Ee(t):T.tab==="users"&&await ue(t)}catch(s){t.innerHTML=`<div class="empty"><h3>Something went wrong</h3>${escapeHtml(s.message)}</div>`}}}function Z(){return renderPeriodPicker(T.period)}function X(){document.querySelectorAll("[data-period]").forEach(t=>{t.onclick=()=>{T.period=t.dataset.period,h()}})}async function be(t){M=await apiGet("/dashboard/admin?period="+T.period);const s=M.overview;t.innerHTML=`
    ...
    <div class="grid grid-2">
      <section class="block">
        ...
        <div class="card">${se(M.clients.slice(0,8).map(e=>[e.name,e.value]))}</div>
      </section>
      <section class="block">
        <h2>Work Value by Service</h2>
        <div class="card">${se(M.services.slice(0,8).map(e=>[e.name,e.value]),!0)}</div>
      </section>
    </div>
  ...
  `,X()}function se(t,s){if(!t.length)return'<div class="empty">No data yet.</div>';const e=Math.max(1,...t.map(a=>a[1]));return t.map(([a,n])=>`
    <div class="bar-row">
    ...
    </div>`).join("")}function ye(t){const s=Math.min(t.utilization,160);return`
    ...
    </div>`}function O(t){(!g.assignments||g.assignments.every(e=>!e.personId))&&(g.assignments=V());const s=g.assignments.reduce((e,a)=>e+(Number(a.percent)||0),0);t.innerHTML=`
    <section class="block">
      ...
      <div class="card log-job-card">
          ...
          <div class="field" style="margin-bottom:16px;">
            ...
            ${g.serviceIds.length?`
              <div class="selected-services-tags" style="display:flex;flex-wrap:wrap;gap:8px;margin-top:10px;">
                ${g.serviceIds.map(e=>{const a=x.services.find(n=>n._id===e);return a?`
                    <span class="badge blue" style="display:inline-flex;align-items:center;gap:6px;padding:6px 12px;font-size:12px;border-radius:20px;">
                      ${escapeHtml(a.name)}
                      <span class="remove-service-tag" data-id="${a._id}" style="cursor:pointer;font-weight:bold;margin-left:4px;" title="Remove service">✕</span>
                    </span>
                  `:""}).join("")}
              </div>
            `:""}
        ...
        <div style="margin-bottom:24px;">
          ...
          <div class="people-assigned-box">
            ...
            <div id="assignRows">
              ${g.assignments.map((e,a)=>fe(e,a)).join("")}
            </div>
  ...
  `}function he(){const t=document.getElementById("jClient");t&&(t.onchange=v=>g.clientId=v.target.value);const s=document.getElementById("addNewClientBtn");s&&(s.onclick=()=>{const v=openModal(`
    ...
    <section class="block">
      ...
      <div class="card" style="overflow-x:auto;">
        <table>
          ...
          <tbody>
          ${s.map(e=>{const a=e.status==="Completed",n=(e.assignments||[]).map(u=>`${escapeHtml(pe(u.personId))} (${u.percent}%)`).join(", "),l=(e.attachments||[]).length,r=(e.deliverables||[]).length;return`<tr>
              ...
              <td>
                <div style="display:flex;flex-direction:column;gap:3px;align-items:flex-start">
                  <button class="btn ${a?"ghost":"gold"} small toggle-status-btn" data-id="${e._id}" data-done="${a}" style="padding:2px 7px;font-size:11px;cursor:pointer;">
                    <span class="badge ${a?"green":e.status==="Needs Revision"?"red":"amber"}">${a?"Completed":e.status==="Needs Revision"?"Needs Revision":"In Progress"}</span>
                  </button>
  ...
  `,document.querySelectorAll("[data-jf]").forEach(e=>e.onclick=()=>{T.jobsFilter=e.dataset.jf,h()}),document.querySelectorAll(".view-job-files").forEach(e=>{e.onclick=()=>{const a=e.dataset.id,n=s.find(b=>b._id===a);if(!n)return;const l=encodeURIComponent(JSON.stringify(n.attachments||[])),r=encodeURIComponent(JSON.stringify(n.deliverables||[])),u=openModal(`
      ...
      `);bindAttachmentUploader("mDelUpload"),u.querySelector("#mDelSaveBtn").onclick=async()=>{const b=getUploaderAttachments("mDelUpload"),$=u.querySelector("#mDelNote").value.trim();if(!b.length){flashToast("Please select at least one deliverable file",!0);return}try{const I=b.map(v=>({...v,notes:$}));await apiPost(`/jobs/${a}/deliverables`,{deliverables:I}),flashToast("Deliverables added! 📦"),u.remove(),h()}catch(I){flashToast(I.message,!0)}},u.querySelector("#mCloseFiles").onclick=()=>u.remove()}}),document.querySelectorAll(".view-job-tickets").forEach(e=>{e.onclick=()=>{const a=e.dataset.id,n=e.dataset.title,l=openModal(`
        <div style="margin-bottom:12px">
          <h3 style="margin-bottom:4px">🎫 Support Tickets</h3>
          <div style="font-size:12px;color:var(--text-3)">${escapeHtml(n)}</div>
        </div>
        ${renderSupportTicketSection(a,!0)}
        <div class="modal-actions" style="margin-top:16px">
          <button class="btn ghost" id="mCloseTickets">Close</button>
        </div>
      `);bindSupportTicketSection(a,!0),l.querySelector("#mCloseTickets").onclick=()=>l.remove()}}),document.querySelectorAll(".edit-job, .assign-cell-click").forEach(e=>{e.onclick=()=>{const a=e.dataset.id,n=s.find(l=>l._id===a);n&&we(n,()=>h())}}),document.querySelectorAll(".toggle-status-btn").forEach(e=>e.onclick=async()=>{const a=e.dataset.id,n=e.dataset.done==="false",l=s.find(u=>u._id===a);let r=l?l.completionDate:null;n&&!r&&(r=new Date().toISOString().slice(0,10)),await apiPut("/jobs/"+a,{status:n?"Completed":"In Progress",completionDate:r}),flashToast(n?"Job marked as Completed!":"Job marked as In Progress"),h()}),document.querySelectorAll(".del-job").forEach(e=>e.onclick=async()=>{confirm("Delete this job?")&&(await apiDelete("/jobs/"+e.dataset.id),flashToast("Deleted"),h())})}let F="",U="all";async function J(t){let s=await apiGet("/tickets");s.sort((i,m)=>new Date(m.createdAt)-new Date(i.createdAt));const e=s.length,a=s.filter(i=>i.status==="Open").length,n=s.filter(i=>i.status==="In Review").length,l=s.filter(i=>i.status==="Resolved"||i.status==="Closed").length,r=e>0?Math.round(l/e*100):100,u=T.ticketsFilter||"all";let b=s;if(u==="open"?b=b.filter(i=>i.status==="Open"):u==="in-review"?b=b.filter(i=>i.status==="In Review"):u==="resolved"?b=b.filter(i=>i.status==="Resolved"):u==="closed"&&(b=b.filter(i=>i.status==="Closed")),U!=="all"&&(b=b.filter(i=>i.priority===U)),F){const i=F.toLowerCase();b=b.filter(m=>{const D=m.jobId&&m.jobId.title||"";return(m.subject||"").toLowerCase().includes(i)||(m.message||"").toLowerCase().includes(i)||(m.userName||"").toLowerCase().includes(i)||D.toLowerCase().includes(i)})}const $={Open:"red","In Review":"amber",Resolved:"green",Closed:"gray"},I={Low:"green",Medium:"gray",High:"amber",Urgent:"red"};function v(i){if(!i)return"U";const m=i.trim().split(/\s+/);return m.length===1?m[0].slice(0,2).toUpperCase():(m[0][0]+m[m.length-1][0]).toUpperCase()}function d(i){if(!i)return"";const m=new Date,D=new Date(i),N=Math.floor((m-D)/1e3);if(N<60)return"Just now";const H=Math.floor(N/60);if(H<60)return`${H}m ago`;const B=Math.floor(H/60);if(B<24)return`${B}h ago`;const o=Math.floor(B/24);return o<7?`${o}d ago`:fmtDate(i)}t.innerHTML=`
    <section class="block">
      <!-- Top Metrics Hub -->
      <div class="ticket-hub-kpis">
        ...
        <div class="ticket-kpi-card">
          ...
          <div>
            <div class="ticket-kpi-val">${a}</div>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:10px">
        <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
          ...
          <button class="pchip ${u==="open"?"active":""}" data-tf="open">🔴 Open (${a})</button>
      ...
      <div style="display:flex;flex-direction:column;gap:14px">
        ${b.map(i=>{const m=i.jobId?i.jobId.title||"Untitled Job":"General Workspace Support",D=(i.status||"Open").toLowerCase().replace(" ","-"),N=i.status==="Open",H=(i._id||"").slice(-4).toUpperCase(),B=v(i.userName);return`
            ...
            <div class="ticket-card-header">
              ...
              <div class="ticket-meta-badges">
                <span class="badge ${$[i.status]||"gray"}">
                  ${N?'<span class="pulse-dot"></span>':""} ${escapeHtml(i.status)}
                </span>
  ...
  `),r=l.querySelector(".modal");r&&(r.style.maxWidth="680px"),bindAttachmentUploader("mEditAttachments",{existing:t.attachments||[]}),bindAttachmentUploader("mEditDeliverables",{existing:t.deliverables||[]});const u=()=>{const o=e.assignments.reduce((P,j)=>P+(Number(j.percent)||0),0),S=l.querySelector("#mAssignTotalInfo");S&&(S.innerHTML=o===100?'<span style="color:var(--green, #10b981);">✓ 100% Allocated</span>':`<span style="color:var(--amber, #f59e0b);">⚠️ ${o}% Allocated (should be 100%)</span>`)},b=()=>{l.querySelectorAll(".m-remove-service").forEach(o=>{o.onclick=()=>{const S=o.dataset.id;e.serviceIds=e.serviceIds.filter(P=>String(P)!==String(S)),l.querySelector("#mEditServicesTags").innerHTML=a(),b()}})},$=()=>{l.querySelectorAll(".m-person-assign-row").forEach(o=>{const S=Number(o.dataset.idx),P=o.querySelector(".m-a-person");P&&(P.onchange=E=>e.assignments[S].personId=E.target.value);const j=o.querySelector(".m-a-percent");j&&(j.oninput=E=>{e.assignments[S].percent=E.target.value,u()});const c=o.querySelector(".m-a-hours");c&&(c.oninput=E=>e.assignments[S].hours=E.target.value);const A=o.querySelector(".m-a-remove");A&&(A.onclick=()=>{e.assignments.length>1?(e.assignments.splice(S,1),l.querySelector("#mEditAssignRows").innerHTML=n(),$(),u()):flashToast("Job must have at least one team member row",!0)})})},I=l.querySelector("#mCloseJobEdit"),v=l.querySelector("#mEditCancel");I&&(I.onclick=()=>l.remove()),v&&(v.onclick=()=>l.remove());const d=l.querySelector("#mEditTitle");d&&(d.oninput=o=>e.title=o.target.value);const k=l.querySelector("#mEditClient");k&&(k.onchange=o=>e.clientId=o.target.value);const f=l.querySelector("#mEditStatus");f&&(f.onchange=o=>e.status=o.target.value);const p=l.querySelector("#mEditDate");p&&(p.onchange=o=>e.date=o.target.value);const y=l.querySelector("#mEditCompletionDate");y&&(y.onchange=o=>e.completionDate=o.target.value);const i=l.querySelector("#mEditPriority");i&&(i.onchange=o=>e.priority=o.target.value);const m=l.querySelector("#mEditValue");m&&(m.oninput=o=>e.value=o.target.value);const D=l.querySelector("#mEditDesc");D&&(D.oninput=o=>e.description=o.target.value);const N=l.querySelector("#mEditServiceSelect");N&&(N.onchange=o=>{const S=o.target.value;S&&!e.serviceIds.map(String).includes(String(S))&&(e.serviceIds.push(S),l.querySelector("#mEditServicesTags").innerHTML=a(),b()),N.value=""}),b(),$(),u();const H=l.querySelector("#mAddAssignRow");H&&(H.onclick=()=>{e.assignments.push({personId:"",percent:0,hours:0}),l.querySelector("#mEditAssignRows").innerHTML=n(),$(),u()});const B=l.querySelector("#mEditSave");B&&(B.onclick=async()=>{try{if(!e.clientId){flashToast("Client is required",!0);return}if(!e.serviceIds||!e.serviceIds.length){flashToast("At least one service is required",!0);return}const o=e.assignments.filter(C=>C.personId&&String(C.personId).trim()!=="");if(!o.length){flashToast("Please select at least one assigned team member",!0);return}for(const C of o)if(C.hours===""||C.hours==null){flashToast("Enter hours spent for every assigned person",!0);return}const S=x.clients.find(C=>String(C._id)===String(e.clientId)),j=x.services.filter(C=>e.serviceIds.map(String).includes(String(C._id))).map(C=>C.name).join(", "),c=e.title||(S?`${S.name} — ${j||"Deliverable"}`:j||"Untitled Job"),A=getUploaderAttachments("mEditAttachments"),E=getUploaderAttachments("mEditDeliverables");await apiPut("/jobs/"+t._id,{title:c,clientId:e.clientId,serviceIds:e.serviceIds,date:e.date,completionDate:e.completionDate||null,status:e.status,priority:e.priority,value:Number(e.value)||0,description:e.description||"",assignments:o,attachments:A,deliverables:E});const R=o.map(C=>pe(C.personId)).join(", ");flashToast("Job updated & saved! 📁"),l.remove(),s&&s()}catch(o){flashToast(o.message,!0)}finally{B.disabled=!1,B.textContent="Save & Update Assignments"}})}async function $e(t){M=await apiGet("/dashboard/admin?period="+T.period),t.innerHTML=`${Z()}
    ...
    <section class="block"><h2>By Person</h2>
      <div class="card table-card" style="padding:0;overflow:hidden">
        <div class="table-wrapper">
          ...
          <tbody>${M.personnel.map(s=>{const e=(s.status||"active").toLowerCase(),a=s.personId||s._id;return`<tr>
              <td style="padding-left:22px"><strong>${escapeHtml(s.name)}</strong></td><td class="muted">${escapeHtml(s.duties||"")}</td>
              <td>
                <select class="person-status-sel" data-id="${a}" style="padding:4px 10px;font-size:12.5px;font-weight:700;border-radius:6px;border:1px solid var(--border-sm);background:var(--bg-input);color:var(--text-1);cursor:pointer;">
    ...
    </section>`,X(),document.querySelectorAll(".person-status-sel").forEach(s=>{s.onchange=async e=>{const a=s.dataset.id,n=e.target.value;try{await apiPut("/personnel/"+a,{status:n}),await _(),flashToast("Status updated to "+n),me(t)}catch(l){flashToast(l.message,!0)}}})}const G=[["strategy","Strategy"],["cs","CS"],["website","Website"],["design","Design"],["copy","Copy"],["edit","Edit"],["shoot","Shoot"],["seo","SEO"],["smo","SMO"],["qc","QC"]];async function Se(t){const s=await apiGet("/roster");t.innerHTML=`
    <section class="block">
      ...
      <div class="card table-card" style="padding:0;overflow:hidden">
        <div class="table-wrapper">
          ...
          <tbody>${s.map(e=>`<tr>
            ...
            ${G.map(a=>`<td>${escapeHtml(e.roles[a[0]]||"—")}</td>`).join("")}
    ...
    </section>`,document.getElementById("addAccountBtn").onclick=()=>ae(null),document.querySelectorAll(".edit-roster").forEach(e=>e.onclick=()=>ae(s.find(a=>a._id===e.dataset.id)))}function ae(t){const s=!t,e=t||{_id:null,clientId:"",nature:"Existing",roles:{},difficulty:5,comments:""};G.forEach(n=>{e.roles[n[0]]==null&&(e.roles[n[0]]="")});const a=openModal(`
    <h3>${s?"Add Account":"Edit Account"}</h3>
    <div class="field-row">
      <div class="field"><label>Client</label>
        ${s?`<select id="rClient"><option value="">Select…</option>${x.clients.map(n=>`<option value="${n._id}">${escapeHtml(n.name)}</option>`).join("")}</select>`:`<input type="text" value="${escapeHtml(W(e.clientId))}" disabled>`}
      </div>
    ...
    </div>`);a.querySelector("#mCancel").onclick=()=>a.remove(),s||(a.querySelector("#mDelete").onclick=async()=>{confirm("Remove this account?")&&(await apiDelete("/roster/"+e._id),a.remove(),h())}),a.querySelector("#mSave").onclick=async()=>{const n={};a.querySelectorAll(".r-role").forEach(r=>n[r.dataset.key]=r.value.trim());const l={nature:a.querySelector("#rNature").value,roles:n,difficulty:Number(a.querySelector("#rDifficulty").value)||1,comments:a.querySelector("#rComments").value.trim()};try{if(s){const r=a.querySelector("#rClient").value;if(!r){flashToast("Select a client",!0);return}await apiPost("/roster",Object.assign({clientId:r},l))}else await apiPut("/roster/"+e._id,l);a.remove(),h()}catch(r){flashToast(r.message,!0)}}}function Te(t){if(!t)return"Deliverables";const s={count:"Deliverables",hours:"Hours",reels:"Reels",stories:"Stories",posts:"Posts"};return s[t]?s[t]:t.charAt(0).toUpperCase()+t.slice(1)}async function Ie(t){const s=await apiGet("/targets"),e={day:"Daily",week:"Weekly",month:"Monthly"};t.innerHTML=`
    <section class="block">
      ...
      <div class="card table-card" style="padding:0;overflow:hidden">
        <div class="table-wrapper">
          <table style="width:100%;border-collapse:collapse;table-layout:fixed;min-width:780px">
            ...
            <tbody>
              ${s.map(a=>{var f,p;const n=((f=a.personId)==null?void 0:f.name)||"—",l=((p=a.serviceId)==null?void 0:p.name)||"—",r=Te(a.unit),u=e[a.period]||(a.period?a.period.toUpperCase():"Daily"),b=a.actual||0,$=a.quantity>0?b/a.quantity:0,I=Math.round($*100),v=$>=1?"green":$>=.6?"amber":"red",d=$>=1?"✓ Met":$>=.6?"Behind":"Off Pace",k=Math.min(I,100);return`
                <tr>
                  ...
                  <td><span style="font-size:14px;font-weight:700;color:var(--text-1)">${a.quantity}</span></td>
                  ...
                  <td>
                    <div style="display:flex;flex-direction:column;gap:4px">
                      <div style="display:flex;justify-content:space-between;align-items:center;font-size:11.5px">
                        <span style="font-weight:700;color:var(--text-1)">${b} / ${a.quantity} <span style="font-weight:500;color:var(--text-3)">(${I}%)</span></span>
                  ...
                  <td class="num" style="padding-right:22px;white-space:nowrap;text-align:right">
                    <div style="display:inline-flex;gap:6px;align-items:center;justify-content:flex-end">
                      <button class="btn ghost small edit-target" data-id="${a._id}" style="padding:4px 8px;font-size:11.5px">Edit</button>
                      <button class="btn danger small del-target" data-id="${a._id}" style="padding:4px 8px;font-size:11.5px">Remove</button>
                    </div>
                  </td>
  ...
  `,document.getElementById("addAdminTargetBtn").onclick=()=>ie(null),document.querySelectorAll(".edit-target").forEach(a=>{a.onclick=()=>ie(s.find(n=>n._id===a.dataset.id))}),document.querySelectorAll(".del-target").forEach(a=>{a.onclick=async()=>{if(confirm("Remove this target?"))try{await apiDelete("/targets/"+a.dataset.id),flashToast("Target removed"),h()}catch(n){flashToast(n.message,!0)}}})}function ie(t){var I,v,d,k;const s=!t;t=t||{personId:(I=x.personnel[0])==null?void 0:I._id,serviceId:(v=x.services[0])==null?void 0:v._id,quantity:5,unit:"reels",period:"day"};const e=((d=t.personId)==null?void 0:d._id)||t.personId||"",a=((k=t.serviceId)==null?void 0:k._id)||t.serviceId||"",l=["reels","stories","posts","count","hours"].includes(t.unit),r=openModal(`
    ...
    <div class="field" style="margin-bottom:12px">
      ...
      <select id="mTgtService">
        ${x.services.map(f=>`<option value="${f._id}" ${String(f._id)===String(a)?"selected":""}>${escapeHtml(f.name)}</option>`).join("")}
      </select>
    </div>
  ...
  `);bindAttachmentUploader("mTgtAttachments",{existing:t.attachments||[]});const u=r.querySelector("#mTgtUnit"),b=r.querySelector("#mTgtCustomUnitWrap"),$=r.querySelector("#mTgtCustomUnit");u.onchange=()=>{u.value==="custom"?(b.style.display="flex",$.focus()):b.style.display="none"},r.querySelector("#mCancel").onclick=()=>r.remove(),r.querySelector("#mSave").onclick=async()=>{let f=u.value;f==="custom"&&(f=$.value.trim()||"Deliverables");const p=getUploaderAttachments("mTgtAttachments"),y={personId:r.querySelector("#mTgtPerson").value,serviceId:r.querySelector("#mTgtService").value,quantity:Number(r.querySelector("#mTgtQty").value)||1,unit:f,period:r.querySelector("#mTgtPeriod").value,attachments:p};if(!y.personId||!y.serviceId){flashToast("Person and Service are required",!0);return}try{s?await apiPost("/targets",y):await apiPut("/targets/"+t._id,y),flashToast("Target saved successfully! 🎯"),r.remove(),h()}catch(i){flashToast(i.message,!0)}}}async function Ae(t){const[s,e]=await Promise.all([apiGet("/salary/grades"),apiGet("/salary/assignments")]);x.salaryGrades=s,x.salaryAssignments=e;const a={};e.forEach(n=>a[n.personId]=n.gradeId),t.innerHTML=`
    <section class="block">
      ...
      <tbody>${x.personnel.map(n=>`<tr><td><strong>${escapeHtml(n.name)}</strong><div class="muted">${escapeHtml(n.duties||"")}</div></td>
        ...
        <td class="num"><select class="grade-select" data-id="${n._id}"><option value="">Not set</option>${s.map(l=>`<option value="${l._id}" ${a[n._id]===l._id?"selected":""}>${escapeHtml(l.label)}</option>`).join("")}</select></td>
      </tr>`).join("")}</tbody></table></div>
    </section>`,document.getElementById("addGradeBtn").onclick=()=>le(null),document.querySelectorAll(".edit-grade").forEach(n=>n.onclick=()=>le(s.find(l=>l._id===n.dataset.id))),document.querySelectorAll(".del-grade").forEach(n=>n.onclick=async()=>{confirm("Remove grade?")&&(await apiDelete("/salary/grades/"+n.dataset.id),h())}),document.querySelectorAll(".grade-select").forEach(n=>{n.onchange=async()=>{await apiPut("/salary/assignments/"+n.dataset.id,{gradeId:n.value||null}),flashToast("Saved")}})}function le(t){const s=!t;t=t||{label:"",min:0,max:0};const e=openModal(`<h3>${s?"Add":"Edit"} Grade</h3>
    ...
    <div class="modal-actions"><button class="btn ghost" id="mCancel">Cancel</button><button class="btn gold" id="mSave">Save</button></div>`);e.querySelector("#mCancel").onclick=()=>e.remove(),e.querySelector("#mSave").onclick=async()=>{const a={label:e.querySelector("#gLabel").value.trim(),min:Number(e.querySelector("#gMin").value)||0,max:Number(e.querySelector("#gMax").value)||0};if(!a.label){flashToast("Label required",!0);return}try{s?await apiPost("/salary/grades",a):await apiPut("/salary/grades/"+t._id,a),e.remove(),h()}catch(n){flashToast(n.message,!0)}}}function Ce(t){const s={active:"green","work from home":"blue",wfh:"blue","on leave":"amber",inactive:"gray"};t.innerHTML=`
    <section class="block">
      <!-- Personnel Section -->
  ...
  `;const e=document.getElementById("addPersonBtn");e&&(e.onclick=()=>ne(null)),document.querySelectorAll(".edit-person").forEach(l=>l.onclick=()=>ne(x.personnel.find(r=>r._id===l.dataset.id))),document.querySelectorAll(".del-person").forEach(l=>l.onclick=async()=>{confirm("Remove this person?")&&(await apiDelete("/personnel/"+l.dataset.id),await _(),h())});const a=document.getElementById("addClientBtn");a&&(a.onclick=()=>oe(null)),document.querySelectorAll(".edit-client").forEach(l=>l.onclick=()=>oe(x.clients.find(r=>r._id===l.dataset.id))),document.querySelectorAll(".del-client").forEach(l=>l.onclick=async()=>{confirm("Remove this client?")&&(await apiDelete("/clients/"+l.dataset.id),await _(),h())});const n=document.getElementById("addServiceBtn");n&&(n.onclick=()=>re(null)),document.querySelectorAll(".edit-service").forEach(l=>l.onclick=()=>re(x.services.find(r=>r._id===l.dataset.id))),document.querySelectorAll(".del-service").forEach(l=>l.onclick=async()=>{confirm("Remove this service?")&&(await apiDelete("/services/"+l.dataset.id),await _(),h())})}const Ee=Ce;function ne(t){const s=!t;t=t||{name:"",duties:"",capacity:48,status:"active",attachments:[]};const e=openModal(`
  ...
  `);bindAttachmentUploader("mPersonFiles",{existing:t.attachments||[]}),e.querySelector("#mCancel").onclick=()=>e.remove(),e.querySelector("#mSave").onclick=async()=>{const a=getUploaderAttachments("mPersonFiles"),n={name:e.querySelector("#mName").value.trim(),duties:e.querySelector("#mDuties").value.trim(),capacity:Number(e.querySelector("#mCapacity").value)||48,status:e.querySelector("#mStatus").value,attachments:a};if(!n.name){flashToast("Name is required",!0);return}try{s?await apiPost("/personnel",n):await apiPut("/personnel/"+t._id,n),await _(),flashToast("Person saved! 📁"),e.remove(),h()}catch(l){flashToast(l.message,!0)}}}function oe(t){const s=!t;t=t||{name:"",notes:"",attachments:[]};const e=openModal(`
  ...
  `);bindAttachmentUploader("mClientFiles",{existing:t.attachments||[]}),e.querySelector("#mCancel").onclick=()=>e.remove(),e.querySelector("#mSave").onclick=async()=>{const a=getUploaderAttachments("mClientFiles"),n={name:e.querySelector("#mCName").value.trim(),notes:e.querySelector("#mCNotes").value.trim(),attachments:a};if(!n.name){flashToast("Name is required",!0);return}try{s?await apiPost("/clients",n):await apiPut("/clients/"+t._id,n),await _(),flashToast("Client saved! 📁"),e.remove(),h()}catch(l){flashToast(l.message,!0)}}}function re(t){const s=!t;t=t||{name:"",hours:4};const e=openModal(`
  ...
  `);e.querySelector("#mCancel").onclick=()=>e.remove(),e.querySelector("#mSave").onclick=async()=>{const a={name:e.querySelector("#mSName").value.trim(),hours:Number(e.querySelector("#mSHours").value)||0};if(!a.name){flashToast("Name is required",!0);return}try{s?await apiPost("/services",a):await apiPut("/services/"+t._id,a),await _(),e.remove(),h()}catch(n){flashToast(n.message,!0)}}}async function ue(t){const s=await apiGet("/users");let e="all",a="role",n="";function l(){let d=[...s];if(n){const p=n.toLowerCase();d=d.filter(y=>y.name&&y.name.toLowerCase().includes(p)||y.email&&y.email.toLowerCase().includes(p)||y.role&&y.role.toLowerCase().includes(p))}e==="admin"?d=d.filter(p=>p.role==="superadmin"||p.role==="admin"):e==="employee"?d=d.filter(p=>p.role==="employee"):e==="client"&&(d=d.filter(p=>p.role==="client"));const k={superadmin:1,admin:1,employee:2,client:3};a==="role"?d.sort((p,y)=>{const i=k[p.role]||9,m=k[y.role]||9;return i!==m?i-m:(p.name||"").localeCompare(y.name||"")}):a==="role-desc"?d.sort((p,y)=>{const i=k[p.role]||9,m=k[y.role]||9;return i!==m?m-i:(p.name||"").localeCompare(y.name||"")}):a==="name-asc"?d.sort((p,y)=>(p.name||"").localeCompare(y.name||"")):a==="name-desc"?d.sort((p,y)=>(y.name||"").localeCompare(p.name||"")):a==="status"&&d.sort((p,y)=>(y.active?1:0)-(p.active?1:0));const f=t.querySelector("#userTableBody");if(f){if(d.length===0){f.innerHTML='<tr><td colspan="6"><div class="empty" style="padding:32px 16px">No users found matching the selected filters.</div></td></tr>';return}f.innerHTML=d.map(p=>{const y=p.role==="superadmin"||p.role==="admin",i=p.role==="employee",m=y?'<span class="badge gold" style="font-weight:700">👑 Admin</span>':i?'<span class="badge blue" style="font-weight:700">💼 Employee</span>':'<span class="badge green" style="font-weight:700">🤝 Client</span>',D=p.personnelId?escapeHtml(p.personnelId.name):p.clientId?escapeHtml(p.clientId.name):'<span class="muted">—</span>';return`
        <tr>
        ...
        </tr>`}).join(""),f.querySelectorAll(".edit-user").forEach(p=>p.onclick=()=>de(s.find(y=>y._id===p.dataset.id))),f.querySelectorAll(".del-user").forEach(p=>p.onclick=async()=>{if(confirm("Permanently remove this user account?"))try{await apiDelete("/users/"+p.dataset.id),flashToast("User removed"),ue(t)}catch(y){flashToast(y.message,!0)}})}}const r=s.filter(d=>d.role==="superadmin"||d.role==="admin").length,u=s.filter(d=>d.role==="employee").length,b=s.filter(d=>d.role==="client").length;t.innerHTML=`
    <section class="block">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:12px">
        <div>
    ...
    </section>`,l(),t.querySelectorAll(".user-role-filter").forEach(d=>{d.onclick=()=>{t.querySelectorAll(".user-role-filter").forEach(k=>k.classList.remove("active")),d.classList.add("active"),e=d.dataset.role,l()}});const $=t.querySelector("#userSortSel");$&&($.onchange=()=>{a=$.value,l()});const I=t.querySelector("#userSearchInp");I&&(I.oninput=()=>{n=I.value.trim(),l()});const v=t.querySelector("#addUserBtn");v&&(v.onclick=()=>de(null))}function de(t){const s=!t;t=t||{name:"",email:"",role:"employee",personnelId:"",clientId:"",active:!0};const e=t.role,a=openModal(`<h3>${s?"Add":"Edit"} User</h3>
    <div class="field-row">
    ...
    <div class="modal-actions"><button class="btn ghost" id="mCancel">Cancel</button><button class="btn gold" id="mSave">Save</button></div>`);function n(){const l=a.querySelector("#uRole").value,r=a.querySelector("#uLinkWrap");l==="employee"?r.innerHTML=`<label>Linked Personnel</label><select id="uPersonnel"><option value="">Select…</option>${x.personnel.map(u=>`<option value="${u._id}" ${t.personnelId&&(t.personnelId._id||t.personnelId)===u._id?"selected":""}>${escapeHtml(u.name)}</option>`).join("")}</select>`:l==="client"?r.innerHTML=`<label>Linked Client</label><select id="uClient"><option value="">Select…</option>${x.clients.map(u=>`<option value="${u._id}" ${t.clientId&&(t.clientId._id||t.clientId)===u._id?"selected":""}>${escapeHtml(u.name)}</option>`).join("")}</select>`:r.innerHTML=""}a.querySelector("#uRole").onchange=n,n(),a.querySelector("#mCancel").onclick=()=>a.remove(),a.querySelector("#mSave").onclick=async()=>{var b,$;const l=a.querySelector("#uRole").value,r={name:a.querySelector("#uName").value.trim(),email:a.querySelector("#uEmail").value.trim(),role:l,active:a.querySelector("#uActive").checked,personnelId:l==="employee"&&((b=a.querySelector("#uPersonnel"))==null?void 0:b.value)||null,clientId:l==="client"&&(($=a.querySelector("#uClient"))==null?void 0:$.value)||null},u=a.querySelector("#uPassword").value;if(u&&(r.password=u),!r.name||!r.email){flashToast("Name and email required",!0);return}if(s&&!u){flashToast("Password required for a new user",!0);return}try{s?await apiPost("/users",r):await apiPut("/users/"+t._id,r),a.remove(),h()}catch(I){flashToast(I.message,!0)}}}let w={date:new Date().toISOString().slice(0,10),personnelId:"all",filter:"all",search:""};function ce(t,s){let e;if(!t||t==="all")e=new Date;else{const r=t.split("-");r.length===3?e=new Date(Number(r[0]),Number(r[1])-1,Number(r[2])):e=new Date}e.setDate(e.getDate()+s);const a=e.getFullYear(),n=String(e.getMonth()+1).padStart(2,"0"),l=String(e.getDate()).padStart(2,"0");return`${a}-${n}-${l}`}async function ve(t){const[s,e]=await Promise.all([apiGet("/tasks"),apiGet("/personnel").catch(()=>x.personnel||[])]),a=new Date,n=`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`,l=w.date;let r=s;l!=="all"&&(r=s.filter(c=>c.dueDate?new Date(c.dueDate).toISOString().slice(0,10)===l:l===n));let u=r;w.personnelId!=="all"&&(u=u.filter(c=>{var E;const A=((E=c.personnelId)==null?void 0:E._id)||c.personnelId;return String(A)===String(w.personnelId)}));const b=u.length,$=u.filter(c=>c.status==="Completed").length,I=b-$,v=b>0?Math.round($/b*100):0;let d=u;if(w.filter==="active"?d=d.filter(c=>c.status!=="Completed"):w.filter==="completed"&&(d=d.filter(c=>c.status==="Completed")),w.search){const c=w.search.toLowerCase();d=d.filter(A=>{var C,q;const E=(((C=A.personnelId)==null?void 0:C.name)||((q=A.userId)==null?void 0:q.name)||"").toLowerCase(),R=(A.title||"").toLowerCase();return E.includes(c)||R.includes(c)})}const k={};d.forEach(c=>{var C,q,L,z,ee,te;const A=((C=c.personnelId)==null?void 0:C._id)||((q=c.userId)==null?void 0:q._id)||"unknown",E=((L=c.personnelId)==null?void 0:L.name)||((z=c.userId)==null?void 0:z.name)||"Unassigned Employee",R=((ee=c.personnelId)==null?void 0:ee.department)||((te=c.personnelId)==null?void 0:te.role)||"Team Member";k[A]||(k[A]={id:A,name:E,dept:R,tasks:[]}),k[A].tasks.push(c)});const f=Object.values(k),p=l===n;t.innerHTML=`
    <div class="block">
  ...
  `;const y=document.getElementById("admDailyPrevDayBtn");y&&(y.onclick=()=>{w.date=ce(w.date,-1),h()});const i=document.getElementById("admDailyNextDayBtn");i&&(i.onclick=()=>{w.date=ce(w.date,1),h()});const m=document.getElementById("admDailyTodayBtn");m&&(m.onclick=()=>{w.date=n,h()});const D=document.getElementById("admDailyAllDatesBtn");D&&(D.onclick=()=>{w.date=w.date==="all"?n:"all",h()});const N=document.getElementById("admDailyDatePickerInp");N&&(N.onchange=c=>{c.target.value&&(w.date=c.target.value,h())}),document.querySelectorAll("[data-atf]").forEach(c=>{c.onclick=()=>{w.filter=c.dataset.atf,h()}});const H=document.getElementById("admTaskPersonFilter");H&&(H.onchange=c=>{w.personnelId=c.target.value,h()});const B=document.getElementById("admTaskSearchInp");B&&(B.oninput=c=>{w.search=c.target.value,ve(t)});const o=document.getElementById("admTaskClearSearch");o&&(o.onclick=()=>{w.search="",h()});const S=document.getElementById("admDailyTaskInp"),P=document.getElementById("admDailyAddBtn"),j=async()=>{const c=((S==null?void 0:S.value)||"").trim();if(!c){flashToast("Please enter a task title",!0);return}const A=document.getElementById("admDailyAssignSelect"),E=document.getElementById("admDailyAssignDate"),R=document.getElementById("admDailyAssignPriority"),C=(A==null?void 0:A.value)||"all",q=(E==null?void 0:E.value)||(w.date==="all"?n:w.date),L=(R==null?void 0:R.value)||"Medium";try{const z={title:c,dueDate:q,priority:L,status:"Todo"};C==="all"?(z.assignAll=!0,z.personnelId="all"):z.personnelId=C,await apiPost("/tasks",z),flashToast(C==="all"?"Task assigned to all active employees! 👥":"Task assigned! ✍️"),S&&(S.value="",S.focus()),h()}catch(z){flashToast(z.message,!0)}};P&&(P.onclick=j),S&&(S.onkeydown=c=>{c.key==="Enter"&&j()})}window.adminToggleTask=async function(t){try{await apiPatch("/tasks/"+t+"/toggle",{}),flashToast("Task status updated"),h()}catch(s){flashToast(s.message,!0)}};window.adminEditTask=async function(t){try{const e=(await apiGet("/tasks")).find(n=>n._id===t);if(!e)return;const a=prompt("Edit task title:",e.title);a!==null&&a.trim()&&(await apiPut("/tasks/"+t,{title:a.trim()}),flashToast("Task updated"),h())}catch(s){flashToast(s.message,!0)}};window.adminQuickAddTaskForEmp=async function(t){const s=prompt("Enter task for this employee:");if(!(!s||!s.trim()))try{const e=new Date,a=`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}-${String(e.getDate()).padStart(2,"0")}`,n=w.date==="all"?a:w.date;await apiPost("/tasks",{title:s.trim(),personnelId:t,dueDate:n,priority:"Medium",status:"Todo"}),flashToast("Task assigned to employee! ✍️"),h()}catch(e){flashToast(e.message,!0)}};window.adminDeleteTask=async function(t){if(confirm("Delete this task?"))try{await apiDelete("/tasks/"+t),flashToast("Task deleted"),h()}catch(s){flashToast(s.message,!0)}};window.adminClearCompletedTasks=async function(t){if(confirm("Delete completed tasks?"))try{const s={date:w.date};t&&t!=="all"?s.personnelId=t:w.personnelId&&w.personnelId!=="all"&&(s.personnelId=w.personnelId);const e=await apiPost("/tasks/clear-completed",s);flashToast(`Completed tasks deleted (${e.deletedCount||0} removed) 🗑️`),h()}catch(s){flashToast(s.message,!0)}};ge();
